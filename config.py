...
DEFAULTS = {
    ...
    "torrent_port": 0,
    # uTP выключен по умолчанию (замер на настоящем рое, находка 65):
    # голова 2 МБ приходит за 4.71 с вместо 10.25 (Sintel) и 2.35 вместо
    # 8.97 (BBB), разброс между прогонами схлопывается. Цена — пиров на
    # треть меньше (35 -> 28 и 77 -> 48): uTP по устройству уступает
    # дорогу другому трафику, поэтому его пиры приходят последними ровно
    # тогда, когда кусок нужен «сейчас». Замер сделан на одном канале,
    # поэтому это настройка, а не зашитое в код поведение: там, где
    # uTP-пиры единственные доступные, выключение может навредить.
    # Применяется при создании сессии, то есть с перезапуском (как порт).
    "torrent_disable_utp": True,
