...
def watchable_files(files):
    """Что вообще можно смотреть: ВСЕ видеофайлы раздачи.

    Порядок — как в раздаче: у сериала имена серий обычно идут по
    порядку, и пользователю в диалоге (2.2) привычнее видеть их так, а
    не по размеру.

    До 2.5 отбирались только файлы с галочкой — невыбранные libtorrent
    не качает, предлагать их было нечего. С 2.5 «Смотреть» сам
    переключает закачку на выбранную серию (engine.focus_file), и с
    прежним отбором после первого же просмотра в диалоге осталась бы
    одна серия — та, которую уже смотрят. Выбор серии здесь и ЕСТЬ
    команда «качай теперь её».
    """
    return [f for f in files if is_video(f.path)]
...
def choose_video_file(files):
    ...
    самый большой видеофайл раздачи.
