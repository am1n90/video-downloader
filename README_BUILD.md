...
1. Запустить `Output\VideoDownloader-Setup-<APP_VERSION>.exe` → установка без ошибок, ярлыки в меню Пуск (и на рабочем столе, если выбрано).
...
9. Деинсталляция: если программа ещё запущена, деинсталлятор попросит её закрыть (Retry/Cancel) и не начнёт удаление, пока она не закрыта. После подтверждения удаления — отдельный вопрос «Удалить также данные пользователя?»: «Нет» — папка приложения удалена, ярлыки сняты, `%LOCALAPPDATA%\VideoDownloader` остаётся; «Да» — данные тоже удаляются.
