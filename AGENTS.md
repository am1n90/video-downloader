...
- Находки 1.4 (сборка с libtorrent; нумерация с 26):
  26. **libtorrent в сборке — пакет, а не одиночный модуль.** В
      ...
      Про `-selftest`: он теперь проверяет и libtorrent в exe, потому что
      MainWindow СОЗДАЁТ TorrentEngine, — но только загрузку .pyd:
      `start()` оттуда не зовётся (порт и запрос брандмауэра; сессию
      поднимает страница при первом показе), так что «сессия libtorrent
      работает в frozen» этим НЕ доказано. `[InstallDelete]` на
