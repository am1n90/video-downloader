...
- **Следующая сессия — 2.3**: внешние .srt через тот же сервер, MP4 с
  moov в конце и MKV с дорожками на настоящих файлах, длительный
  просмотр, проверка на УСТАНОВЛЕННОЙ копии. По итогам 2.2 туда же —
  вопрос про перезапрос «горящих» кусков (находка 33).
...
- Тесты задачи: `test_mode_switch.py` 51, `test_close_threads.py` 22,
  `test_torrent_engine.py` 58, `test_torrent_gui.py` 100,
  `test_torrent_stream.py` 35 (последний — без libtorrent и без сети, на
  поддельном источнике); живые проверки `check_mode_switch_live.py`,
  `check_torrent_gui_live.py` и `check_torrent_stream_live.py`
...
- Замеры на рое (только домашняя машина): `check_swarm_stream.py`
  (сравнение настроек на синтетическом читателе),
  `check_swarm_player.py` (весь путь с живым VLC),
  `diag_swarm_head.py` (кто виноват в медленной голове — рой или мы).
...
- Просмотр на настоящем рое ПРОВЕРЕН в 2.2 (`check_swarm_player.py`,
  цифры — в «Сессия 2.2 — сделано»), но только на раздачах, которые
  докачиваются за 30-60 с. **Долгий просмотр РЕАЛЬНО недокачанного
  файла не проверялся**: нужна раздача заметно больше домашнего канала
  (или ограничение скорости) — это в 2.3.
- **Подвисания КАРТИНКИ на рое не мерились**: считались ожидания кусков
  в `TorrentStream` и отсутствие ошибок потока в логе плеера, а не то,
  замирало ли изображение. Для вывода «0 подвисаний» нужен разбор лога
  плеера по-другому (в прототипе — замер 9/11).
- Просмотр с mpv не проверялся вживую: на домашней машине его нет,
  найден и использован VLC. Порядок предпочтения в `player.py` — mpv
  первым (по замерам 0.2 он быстрее), то есть на машине с mpv пойдёт
  другая ветка.
...
- НОВОЕ после 2.2: чтение головы упирается в один самый медленный кусок
  из первых, дедлайны порядок не держат (находка 33). Ручки libtorrent
  про перезапрос «горящих» кусков (`piece_timeout`, `request_timeout`,
  end-game) не изучались — кандидат на 2.3.
- Диалог «Что смотреть» проверялся на раздаче из двух видеофайлов в
  одной папке. Сериал с вложенными папками и десятком серий на
  настоящем рое через диалог не открывали.
- Индикатор подготовки: длинное окно (те самые ~20 с из находки 11)
  вживую не воспроизводилось — на этой машине VLC «прогрет» и
  отзывается за 0.1-1.5 с. Пороги 8 и 45 с проверены сдвигом часов в
  тесте, а не настоящим ожиданием.
...
- **Этап 2 (3-4 сессии)**: просмотр во время закачки через внешний
  ...
  - 2.2 — СДЕЛАНО 16.09.2026 (код + тесты + живые проверки + замеры на
    ...
    сделано»
  - 2.3: внешние .srt через тот же сервер, MP4 с moov в конце и MKV с
    дорожками на настоящих файлах, длительный просмотр, проверка на
    УСТАНОВЛЕННОЙ копии
