...
- Установленная копия на этой машине — ещё 1.0.1; живое обновление
  1.0.1→1.0.2 = тест после релиза v1.0.2 (см. «Что осталось» п.1)
...
1. **Живой тест обновления 1.0.1→1.0.2 с GitHub** (после релиза v1.0.2 и
   только после подтверждения владельца): установленная копия 1.0.1
   (history=9, backup settings.json.bak-20260910-233245) — сбросить
   update_manifest_url на дефолтный GitHub URL (сейчас там остаток теста
   127.0.0.1:8765; правка с backup, UTF-8 без BOM, 0 процессов),
   холодный старт → автопроверка → InfoBar 1.0.2 → обновление →
   перезапуск. Проверки после обновления: тег v1.0.2 → коммит main;
   Setup-ассет байт-в-байт + digest; алиас latest отдаёт 1.0.2; версия
   приложения 1.0.2; история 9 → те же 9 (совпадение множества путей);
   настройки целы; состав установленной 1.0.2: deno.exe рядом с exe,
   `_internal\yt_dlp_ejs`, анализ 4K без предупреждения, лог пишется;
   холодный старт ×2 (0 процессов → окно)
2. Чеклист чистой машины (README_BUILD.md): установка, анализ, 1080p
   merge, MP3, пауза/.part, кириллица/пробелы, деинсталляция
3. Перевести build_ffmpeg.ps1 на ASCII (работает, но кириллица
   без BOM читается PS 5.1 как ANSI)
4. Если settings.json не читается и сохранение отключено
   (_skip_saving) — показывать пользователю предупреждение в GUI
   (InfoBar); сейчас это видно только в app.log
5. Защита от второго экземпляра (single instance): иначе последний
   закрытый экземпляр затирает историю другого
6. Дистрибуция: README для пользователей (SmartScreen), при желании
   LICENSE (MIT)
