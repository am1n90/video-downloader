...
5. **1.0.5 Галочка «Без водяного знака» и вход для Instagram** — по
   итогам проверки источников
6. Чеклист чистой машины (README_BUILD.md): установка, анализ, 1080p
   merge, MP3, пауза/.part, кириллица/пробелы, деинсталляция
7. Перевести build_ffmpeg.ps1 на ASCII (работает, но кириллица
   без BOM читается PS 5.1 как ANSI)
8. Если settings.json не читается и сохранение отключено
   (_skip_saving) — показывать пользователю предупреждение в GUI
   (InfoBar); сейчас это видно только в app.log
9. Защита от второго экземпляра (single instance): иначе последний
   закрытый экземпляр затирает историю другого
10. Дистрибуция: README для пользователей (SmartScreen), при желании
   LICENSE (MIT)
