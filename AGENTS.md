...
- **Этап 2: сессия 2.1 — завершена** (код + тесты + живая проверка с
  ...
  в `closeEvent`.
- **Следующая сессия — 2.2**: выбор файла для многофайловых раздач
  (диалог и «Смотреть» в дереве файлов), индикатор подготовки плеера,
  замеры на НАСТОЯЩЕМ рое (только домашняя машина) и ответ на вопросы
  про `min_reconnect_time` (находка 18) и входящие соединения
  (находка 13) для стриминга.
...
- Тесты задачи: `test_mode_switch.py` 51, `test_close_threads.py` 22,
  `test_torrent_engine.py` 58, `test_torrent_gui.py` 76,
  `test_torrent_stream.py` 27 (последний — без libtorrent и без сети, на
  ...
  следующем прогоне восстанавливаются из fastresume.
...
- **Просмотр (2.1) на настоящем рое не проверялся**: живая проверка —
  локальный сид 1.5 МБ/с и VLC на домашней машине (первый кадр: первое
  обращение плеера к серверу 0.1-4.6 с, вычитано 8.8-10.5 МБ за 10-11 с
  при 29% скачанного). Цифры прототипа для роя — 7-8 с до первого
  кадра (RESULTS.md, замер 11), в приложении не подтверждены.
...
- **Этап 2 (3-4 сессии)**: просмотр во время закачки через внешний
  ...
  - 2.2 (следующая): диалог выбора файла для многофайловых раздач и
    «Смотреть» прямо в дереве файлов; индикатор подготовки (первый
    запуск VLC — разовые ~20 с, находка 11); настоящий рой и замеры
    (первый кадр, перемотка, подвисания) — только домашняя машина;
    ответ на вопрос про `min_reconnect_time` (находка 18) и входящие
    (находка 13) для стриминга
