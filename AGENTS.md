...
- **Этап 1: сессии 1.1 и 1.2 — завершены** (код + тесты, в main).
  1.1 — переключатель режимов «Video Downloader | Torrent»;
  1.2 — `torrent_engine.py` (движок без GUI).
- **Следующая сессия — 1.3: GUI режима Torrent поверх движка.** Затем 1.4
  (Библиотека торрентов, сборка с libtorrent, установленная копия).
- **Сборки и релиза с торрент-кодом не было** — это шаг 1.4. Последний
  опубликованный релиз — v1.0.7 (в нём торрент-кода нет).

### Состояние репозитория

- `main == origin/main == d98bb20`, рабочее дерево чистое (16.09.2026).
...
- `gui_torrent.py` — страницы режима Torrent (пока заглушки, их и
  заменяет сессия 1.3).
- `config.py` — ключ `app_mode` в DEFAULTS; `requirements.txt` —
...
- Тесты задачи: `test_mode_switch.py` 51, `test_close_threads.py` 22,
  `test_torrent_engine.py` 41, живая проверка `check_mode_switch_live.py`.
...
### План сессии 1.3 (GUI режима Torrent)

Заменить заглушки в `gui_torrent.py` на рабочие страницы поверх готового
движка. Подводные камни (все проверены в 1.1-1.2):

1. **Один движок на приложение**, создаётся в `MainWindow` рядом с
   `DownloadManager`. Колбэки `on_change`/`on_list_change` приходят ИЗ
   ФОНОВОГО ПОТОКА — виджеты трогать только через сигналы (как `Bridge`).
2. **Добавление**: поле для magnet-ссылки и выбор `.torrent`
   (`add_magnet` / `add_torrent_file`). До метаданных состояние
   `metadata` — показывать «получение сведений о раздаче».
3. **Дерево файлов с галочками**: `TorrentItem.files` (путь, размер,
   приоритет) + `set_files`. `prioritize_files` асинхронный — не
   проверять результат сразу (находка 21). Для пользователя показывать
   `selected_size` (сумма выбранных файлов), не `wanted_size` (он
   считается по целым кускам).
4. **Карточки очереди**: прогресс, скорость, пиры, ETA, индикатор
   раздачи; состояния `metadata/checking/downloading/seeding/finished/
   paused/error`. При `error` — текст и файл из `TorrentItem.error`
   плюс кнопка «Повторить» → `engine.retry()` (иначе libtorrent молчит
   10 минут, находка 2).
5. **Удаление** — по образцу `ConfirmDeleteDialog` Библиотеки, галочка
   «удалить файлы» → `remove(delete_files=True)`; служебный `.parts`
   движок убирает сам.
6. **Настройки, группа «Torrent»**: папка загрузок, раздача после
   скачивания (`set_seed_after_download`, по умолчанию включена —
   решение №4), порт (`listen_interfaces`).
7. **closeEvent**: `engine.shutdown(timeout≈3)` — фактически 0.26-0.38 с
   (сброс кэша + fastresume). Дольше окно не держать.
8. **Тесты 1.3**: offscreen GUI поверх движка с локальным сидом; ждать
   `processEvents + sleep`, НЕ `QTest.qWait`; `check_updates=False`; не
   оставлять работающих QThread к выходу (правила работы).

### Открытые вопросы и что не проверено

- Движок на настоящем рое (автотесты — только 127.0.0.1).
