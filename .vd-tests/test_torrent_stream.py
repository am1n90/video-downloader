...
def main():
    ...
    # 2.5: галочка больше не отбирает — «Смотреть» сам переключает на
    # выбранный файл закачку, поэтому видео без галочки тоже предлагаем
    picked = ts.choose_video_file(files)
    check("20. Смотрим самый большой видеофайл, галочка не важна",
          picked is not None and picked.index == 3,
    ...
    check("20c. Для диалога — все видео в порядке раздачи, txt — нет",
          [f.index for f in watchable] == [0, 1, 3],
